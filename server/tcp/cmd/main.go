...
import (
	...
	"h02-server/server/internal/server"
...
func main() {
	...
	if cfg.DBEnabled {
		...
		log.Info("database disabled — all logins auto-approved")
	}
	...
	fwd := forwarder.NewStream(rdb, cfg.StreamKey, cfg.StreamMaxLen, cfg.OnlineZKey, log)
	m := metrics.NewTCP()
	srv := server.NewTCP(cfg, reg, fwd, devices, m, log)
	...
	if err := srv.Run(ctx); err != nil {
		log.Error("H02 TCP server exited with error", zap.Error(err))
		os.Exit(1)
	}
}
